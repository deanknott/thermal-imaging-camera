...
/* This file contains all the required maths to interpolate an 8x8 array into any multiple of this. 
 * the higher the interpolation the slower the refresh rate on the esp32
 * Produced by Adafruit for the AMG88xx libary
 */
...
/* get a point at an individual coord
 */
float get_point(float *p, uint8_t rows, uint8_t cols, int8_t x, int8_t y) {
  if (x < 0)        x = 0;
  if (y < 0)        y = 0;
...
/* due to the interpolation, setting of point is needed as it is the way to create the new array
 */
void set_point(float *p, uint8_t rows, uint8_t cols, int8_t x, int8_t y, float f) {
 ...
 * calculate the factor of interpolation with mu_x and mu_y
 * use ^^ variables to interpolate 
 */
void interpolate_image(float *src, uint8_t src_rows, uint8_t src_cols, 
                       float *dest, uint8_t dest_rows, uint8_t dest_cols) {
